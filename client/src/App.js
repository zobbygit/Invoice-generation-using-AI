...
const [page, setPage] = useState("landing"); // ← default to landing
  ...
  const addItem = () => {
    setInvoiceData((prev) => ({
      ...prev,
      items: [...prev.items, { description: "", quantity: "", price: "", taxType: "gst18" }],
    }));
  };
  ...
  const resetInvoice = () => {
    if (!window.confirm("Reset the entire form? This will clear your draft.")) return;
  ...
  const calculateTotals = () => {
    ...
    if (invoiceData.taxMode === "split") {
      ...
      invoiceData.items.forEach((it) => {
        ...
        const rateMap = {
          gst0: 0, gst5: 5, gst12: 12, gst18: 18, gst28: 28,
          cgst9: 9, sgst9: 9, igst18: 18,
        };
      ...
      // simple single GST %
      gstAmount = (afterDiscount * (Number(invoiceData.gstPercent) || 0)) / 100;
    }
...
//   "http://localhost:5001/generate-invoice",{
      ...
      // 🔥 THE FIX: Strip out global tags and styles that break the app's theme
 ...
 if (page === "dashboard") {
    return (
      <div className="app-container">
        ...
        <nav className="top-nav">
          ...
          <button className="nav-btn theme-toggle" onClick={() => setDarkMode((d) => !d)} title="Toggle theme">
  ...
  <i className="fas fa-home"></i> Home
      ...
      <header className="app-header">
        ...
        <p className="subtitle">Create stunning, professional invoices in seconds</p>
      </header>
      ...
      <div className="card glass">
        ...
        <div className="form-section">
          ...
          <div className="input-grid">
            ...
            <div className="input-wrapper">
              ...
              <input className="input-field date-field" type="date" name="date" value={invoiceData.invoiceMeta.date} onChange={(e) => handleInputChange(e, null, "invoiceMeta")} />
            </div>
            <div className="input-wrapper">
              <i className="fas fa-calendar-check input-icon"></i>
              <input className="input-field date-field" type="date" name="due" value={invoiceData.invoiceMeta.due} onChange={(e) => handleInputChange(e, null, "invoiceMeta")} />
            </div>
            <div className="input-wrapper">
        ...
        <div className="form-section">
          <div className="items-heading-row">
            ...
            <div className="tax-mode-toggle">
              <button
                className={`pill-btn ${invoiceData.taxMode === "simple" ? "active" : ""}`}
                onClick={() => setInvoiceData((p) => ({ ...p, taxMode: "simple" }))}
              >
                Simple GST
              </button>
              <button
                className={`pill-btn ${invoiceData.taxMode === "split" ? "active" : ""}`}
                onClick={() => setInvoiceData((p) => ({ ...p, taxMode: "split" }))}
              >
                Per-item Tax
              </button>
            </div>
          ...
          <div className={`items-header ${invoiceData.taxMode === "split" ? "with-tax" : ""}`}>
          ...
          {invoiceData.items.map((item, index) => (
            <div
              className={`flex-row item-row ${invoiceData.taxMode === "split" ? "with-tax" : ""}`}
              key={index}
            >
              <input className="input-field" name="description" placeholder="Item description" value={item.description} onChange={(e) => handleInputChange(e, index)} />
              <input className="input-field" name="quantity" type="number" placeholder="0" value={item.quantity} onChange={(e) => handleInputChange(e, index)} />
              <input className="input-field" name="price" type="number" placeholder="0.00" value={item.price} onChange={(e) => handleInputChange(e, index)} />
              ...
              <div className="item-actions">
                <button className="icon-btn" onClick={() => duplicateItem(index)} title="Duplicate">
                  <i className="fas fa-copy"></i>
                </button>
                <button className="icon-btn danger" onClick={() => removeItem(index)} disabled={invoiceData.items.length === 1} title="Remove">
        ...
        <div className="totals">
          ...
          <p><span>Discount</span><span className="negative">-₹{discountAmount.toFixed(2)}</span></p>

          {invoiceData.taxMode === "simple" ? (
            <p><span>GST</span><span>₹{gstAmount.toFixed(2)}</span></p>
          ) : (
            <>
              {(cgstAmount > 0) && <p><span>CGST</span><span>₹{cgstAmount.toFixed(2)}</span></p>}
              {(sgstAmount > 0) && <p><span>SGST</span><span>₹{sgstAmount.toFixed(2)}</span></p>}
              {(igstAmount > 0) && <p><span>IGST</span><span>₹{igstAmount.toFixed(2)}</span></p>}
              <p><span>Total Tax</span><span>₹{gstAmount.toFixed(2)}</span></p>
            </>
          )}

          <h3><span>Total</span><span className="total-amount">₹{total.toFixed(2)}</span></h3>
          <p className="amount-words"><em>{numberToWords(total)}</em></p>
        </div>

        <div className="action-row">
          <button className="button generate-btn" onClick={generateInvoice} disabled={loading}>
            {loading ? (<><i className="fas fa-spinner fa-spin"></i> Generating...</>) : (<><i className="fas fa-rocket"></i> Generate Invoice</>)}
          </button>
          <button className="button reset-btn" onClick={resetInvoice} title="Reset form">
